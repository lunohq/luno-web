...
import resumeReply from 'u/resumeReply'
...
class Reply extends Component {
  ...
  initialize({ reply }) {
    ...
    if (reply || resume) {
      let initialValues
      if (resume) {
        initialValues = { reply: resume }
      } else {
        initialValues = { reply }
      }
      this.context.store.dispatch(initialize(FORM_NAME, initialValues))
    }
    this.setState({ deleting: false, editing: !!resume, focused: false })
  }
  ...
  render() {
    ...
    const overlayOpen = !pristine && (editing || submitting)
    const overlay = (
      <Overlay
        onTouchTap={this.handleOverlayTouchTap}
        show={overlayOpen}
        style={{ zIndex: 1350, backgroundColor: 'rgba(0, 0, 0, 0.3)' }}
      />
    )
    // one more than overlay
    const rootStyle = { zIndex: 1351 }

    return (
      <section className={s.root}>
        {overlay}
        <Paper className={s.replyRoot} style={rootStyle}>
          ...
          <div className={s.content} ref='container'>
            <section className={s.form}>
              ...
              />
              <FieldArray
                enabled={this.props.attachmentsEnabled}
