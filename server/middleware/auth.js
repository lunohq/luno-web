...
import { isBotInstalled, createFileChannel } from '../actions/slack'
import { sendNewTrainerNotification, sendReactivatedNotification } from '../actions/notifications'
import { createOnboardingTemplate } from '../actions/templates'
...
async function updateUserDetails({ user, team }) {
  ...
  if (user.role === db.user.ADMIN && team.status === db.team.STATUS_INACTIVE) {
    try {
      ...
      logger.error('Error with reactivation flow', { err, team, user })
    }
  }
...
function oauth(converse, app) {
  converse.createAuthEndpoints(app, async (err, req, res) => {
    ...
    if (user.scopes.includes('channels:write') && !team.files) {
      try {
    ...
    if (
      isNew.bot &&
      team.files &&
      !team.onboardingTemplate &&
      team.slack && team.slack.bot
    ) {
      debug('Creating onboarding template')
      try {
        await createOnboardingTemplate({
          teamId: team.id,
          channelId: team.files.channelId,
          botId: team.slack.bot.userId,
        })
      } catch (err) {
        logger.error('Error loading onboarding template', { err, user, team })
      }
    }
    ...
    setCookie({ res, token })
    debug('Successful auth, redirecting')
    return res.redirect('/')
  })
}
